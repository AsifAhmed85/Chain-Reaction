        ...
        if(blast == 4) {
            ...
            if(x>0) {
                if(grid[x-1][y] != "No" && grid[x-1][y][0] != color[0])
                {
                    ...
                    {
                        p_cnt--;
                        count += (4 - how_many_tobe_critical(grid, x-1, y, player_color));
                    }
                    grid[x-1][y][0] = color[0];
                }
            ...
            if(y>0) {
                if(grid[x][y-1] != "No" && grid[x][y-1][0] != color[0])
                {
                    ...
                    {
                        p_cnt--;
                        count += (4 - how_many_tobe_critical(grid, x, y-1, player_color));
                    }
                    grid[x][y-1][0] = color[0];
                }
...
int hero_max(string grid[8][8], int x, int y, string color)
{
    ...
    for(int i=0; i<8; i++)
    {
        for(int j=0; j<8; j++)
        {
            if(is_critical(grid, i, j, enemy_color))
            {
                if(i>0) {g += (4-how_many_tobe_critical(grid, i-1, j, player_color));}
                if(i<7) {g += (4-how_many_tobe_critical(grid, i+1, j, player_color));}
                if(j>0) {g += (4-how_many_tobe_critical(grid, i, j-1, player_color));}
                if(j<7) {g += (4-how_many_tobe_critical(grid, i, j+1, player_color));}
            }
        }
    }
    ...
    for(int i=0; i<8; i++)
    {
        for(int j=0; j<8; j++)
        {
            if(is_critical(grid1, i, j, enemy_color))
            {
                if(i>0) {g -= (4-how_many_tobe_critical(grid1, i-1, j, player_color));}
                if(i<7) {g -= (4-how_many_tobe_critical(grid1, i+1, j, player_color));}
                if(j>0) {g -= (4-how_many_tobe_critical(grid1, i, j-1, player_color));}
                if(j<7) {g -= (4-how_many_tobe_critical(grid1, i, j+1, player_color));}
            }
        }
    }
    value += (a - b*4 - c*5 + d - e*2 + f*2 - g*5);
    return value;
}
...
int hero_min(string grid[8][8], int x, int y, string color)
{
    ...
    for(int i=0; i<8; i++)
    {
        for(int j=0; j<8; j++)
        {
            if(is_critical(grid, i, j, player_color))
            {
                if(i>0) {g += (4-how_many_tobe_critical(grid, i-1, j, enemy_color));}
                if(i<7) {g += (4-how_many_tobe_critical(grid, i+1, j, enemy_color));}
                if(j>0) {g += (4-how_many_tobe_critical(grid, i, j-1, enemy_color));}
                if(j<7) {g += (4-how_many_tobe_critical(grid, i, j+1, enemy_color));}
            }
        }
    }
    if(is_critical(grid, x, y, enemy_color)) {
        f ++;
        a = how_many_blast(grid1,x,y,color);
        b = biggest_critical_chain_access(grid1, player_color);
    }
    else
    {
    ...
    for(int i=0; i<8; i++)
    {
        for(int j=0; j<8; j++)
        {
            if(is_critical(grid1, i, j, player_color))
            {
                if(i>0) {g -= (4-how_many_tobe_critical(grid1, i-1, j, enemy_color));}
                if(i<7) {g -= (4-how_many_tobe_critical(grid1, i+1, j, enemy_color));}
                if(j>0) {g -= (4-how_many_tobe_critical(grid1, i, j-1, enemy_color));}
                if(j<7) {g -= (4-how_many_tobe_critical(grid1, i, j+1, enemy_color));}
            }
        }
    }
    value += (a - b*4 - c*5 + d - e*2 +f*2 - g*5);
...
int main(int argc, char* argv[])
{
    ...
    while(command == "start")
    {
        ...
        pair<int , int> co_ordinate = alphaBeta(player_color, grid, 6, 10);
