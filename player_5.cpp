...
int hero_max(string grid[8][8], int x, int y, string color)
{
    ...
    if(is_critical(grid, x, y, player_color)) {
        ...
        value += 20*how_many_blast(grid1,x,y,color);
        value -= 5*biggest_critical_chain_access(grid1, enemy_color);
    }
    else
    {
        if(x>0 && is_critical(grid, x-1, y, enemy_color)) {value -= 6; flag = true;}
        if(x<7 && is_critical(grid, x+1, y, enemy_color)) {value -= 6; flag = true;}
        if(y>0 && is_critical(grid, x, y-1, enemy_color)) {value -= 6; flag = true;}
        if(y<7 && is_critical(grid, x, y+1, enemy_color)) {value -= 6; flag = true;}
        if(flag == false)
        {
            value++;
            if(x==0)
...
int hero_min(string grid[8][8], int x, int y, string color)
{
    ...
    {
        if(x>0 && is_critical(grid, x-1, y, player_color)) {value -= 6; flag = true;}
        if(x<7 && is_critical(grid, x+1, y, player_color)) {value -= 6; flag = true;}
        if(y>0 && is_critical(grid, x, y-1, player_color)) {value -= 6; flag = true;}
        if(y<7 && is_critical(grid, x, y+1, player_color)) {value -= 6; flag = true;}
        if(flag == false)
        {
            value++;
            if(x==0)
