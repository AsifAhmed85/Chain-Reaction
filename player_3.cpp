    ...
    if(grid[x][y] == "No") {
        ...
        if(blast == 4) {
            ...
            if(x>0) {
                ...
                {
                    if(is_corner(x-1, y)) count++;
                    else if(is_edge(x-1, y)) count++;
            ...
            if(y>0) {
                ...
                {
                    if(is_corner(x, y-1)) count++;
                    else if(is_edge(x, y-1)) count++;
...
int how_many_critical_access(string grid[8][8], string color)
{
    int c=0;
    for(int  i=0; i<8; i++) {
        for(int j=0; j<8; j++) {
            if(is_critical(grid, i, j, color)) {
                int x=i, y=j;
                if(x>0) {
                    if(color == player_color && is_critical(grid, x-1, y, enemy_color)) c++;
                    else if(color == enemy_color && is_critical(grid, x-1, y, player_color)) c++;
                }
                if(x<7) {
                    if(color == player_color && is_critical(grid, x+1, y, enemy_color)) c++;
                    else if(color == enemy_color && is_critical(grid, x+1, y, player_color)) c++;
                ...
                if(y>0) {
                    if(color == player_color && is_critical(grid, x, y-1, enemy_color)) c++;
                    else if(color == enemy_color && is_critical(grid, x, y-1, player_color)) c++;
                }
                if(y<7) {
                    if(color == player_color && is_critical(grid, x, y+1, enemy_color)) c++;
                    else if(color == enemy_color && is_critical(grid, x, y+1, player_color)) c++;
...
int hero_max(string grid[8][8], int x, int y, string color)
{
    ...
    if(is_critical(grid, x, y, player_color)) {
        f ++;
        b = how_many_critical_access(grid, enemy_color);
        string grid1[8][8];
        p_cnt = 0, o_cnt = 0;
        for(int j=0; j<8; j++)
        {
        ...
        b -= how_many_critical_access(grid1, enemy_color);
    }
    else
    {
    ...
    value += (a*10 - b*5 - c*5 + d*2 - e*2 +f*2);
    return value;
}
...
int hero_min(string grid[8][8], int x, int y, string color)
{
    ...
    if(is_critical(grid, x, y, enemy_color)) {
        b = how_many_critical_access(grid, player_color);
        f ++;
        string grid1[8][8];
        p_cnt = 0, o_cnt = 0;
        for(int j=0; j<8; j++)
        ...
        a = how_many_blast(grid1,x,y,color);
        b -= how_many_critical_access(grid1, player_color);
    }
    else
    {
...
int main(int argc, char* argv[])
{
    ...
    while(command == "start")
    {
        ...
        pair<int , int> co_ordinate = alphaBeta(player_color, grid, 5, 13);
